...
import numpy as np
from tools.data_loader import SalObjDataset
from tools.u2net import U2NET, U2NETP
from tools.utils import model_evaluate, get_date
...
# 存放损失
log_dir = os.path.join(Path(__file__).resolve().parent, "logs", f"logs-{get_date()}")
...
def train(epochs: int, batch_size: int, load_model_path: str = None, interval: int = 1, start_epoch: int = 1):
    """
    :param epochs: 轮次
    :param batch_size: 批次
    ...
    model_dir = os.path.join(Path(__file__).resolve().parent, 'saved_models', model_name + os.sep)
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        if epoch % interval == 0:
            ...
            model_path = os.path.join(model_dir, model_name + f"-{epoch}_{timestamp}.pth")
...
if __name__ == '__main__':
    load_model_path = None  #第一次训练，则设置为load_model_path=None
    # load_model_path = 'saved_models/u2netp/u2netp-11.pth' # 继续训练，则加载之前保存的模型
    
    start_epoch = 1  # 第一次则从头开始训练，设置 start_epoch 为 1
    total_epochs = 1 #总训练次数

    train(epochs=total_epochs, batch_size=2,
          load_model_path=load_model_path,  #第一次训练，则设置为load_model_path=None,
          interval=1,
